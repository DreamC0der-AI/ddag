...
**Say the round once; say the claim in one sentence.** After a change, several judgments need re-anchoring. What moved and how it was checked are facts about the change, not about any claim: record them once (`round_record`) and cite the round from each re-anchoring, whose evidence is then one sentence about *that* claim — "the registry path did not move; its tests are in the run". The first sentence of any evidence is what the views show, so it must be the one about the claim. Pin what the claim rests on, by passing `artifacts`: explicit artifacts are the whole pin set, and a claim with parts pins none. Before re-judging a claim, read its own chain (`graph_history` with `node`), not the log; to learn what broke one, ask `why`. (Found on this repository's chain: one session pasted a round description into every re-anchoring — evidence tripled in length, each judgment was pinned to eighteen files it did not rest on, every edit then staled thirty judgments, and 60% of the chain became re-anchoring pairs.)

**One target per pipeline.** A part of the build target is something the build is not done without: the audit, the walkthrough, a property. A pipeline that *consumes* the build — publishing it, deploying it, a benchmark campaign, a demo — is a target of its own on the same chain (`target_new`), never a part of the build: a part's wait would read as the build being broken, which is false. A sub-target rests on the build's *state*, a version mark, not on its judgment; its claims may use the build's claims by link, and a claim is judged in one home target only — elsewhere it is used. Targets live on across versions; a retired pipeline is discarded, not a finished one. (Found on this repository's own chain: publish, added under the build, made the build read "broken" for a day while the only missing piece was a catalog listing outside the project; `target_new` with `adopt` moved it out with its judgments intact.)
...
# What a Shipped Target Rests On

Every method in the first audit trees was static or test-shaped: reviews, harnesses, injections, the suite. Nothing in them said "a person who has only the README sits down and uses the thing" — so a root could turn solid on a product nobody had used through its own front door. Nothing in DDAG runs on its own, either: an audit starts when someone asks for one, and a walkthrough was never named. The remedy is not a prompt to remember but leaves that exist from the start, because the only thing the kernel enforces is that a root cannot turn solid over a pending part. **[judgment]**

**A target that ships has two standing parts besides its build, added pending at decomposition time:** an audit decomposed by property (above), and a walkthrough. Their absence is visible on the frontier from the first day; verifying the root over it verifies *built*, not *done*. Write them into the target's own criterion as well — "…, holds under an audit by property, and a user with only the README completes every journey" — so a root judged on build claims alone contradicts its own words.

**The walkthrough is a claim, not a mechanism.** "A new user with only the README can do everything the README says" is a group; each journey a real user takes is a leaf — install from a clean clone, first project to a solid root, a change that goes stale, an audit by the protocol, an issue opened and closed, a version marked — and one leaf per function or panel the journeys do not reach, so every surface is touched at least once. The criterion of each leaf is the observed outcome that counts. The kernel neither knows nor cares that the method behind a leaf is a person clicking around rather than a fuzz harness; a Journey node type would encode method categories into the graph the way a "bug" node would encode findings — the same mistake.

**What makes it a user test rather than another review** is enforced by how the tester session is spawned, not by DDAG:
- *No source.* The tester gets the README, the built product and a clean folder. A tester that reads code is reviewing.
- *The real interface only.* The tool over stdio, the dashboard in a browser, the binary in a scratch HOME — the way a user would meet them.
- *It records; it never fixes.* A journey that works is verified valid with the transcript as evidence — written to a file and cited, so the pin hashes it; one that breaks is refuted and the finding opened with the exact steps and output. Confusing-but-works is an issue at Low. The fix comes from another hand, and a fresh examination reverifies (the fixer never judges the fix).
- *Two chains, honestly.* The tester's workload is its own project with its own chain — a toy built through the tool is not a sub-effort of the tool. The judgments about the product go on the product's chain. When the product under test is DDAG itself, the record is made with the tools under test; if recording breaks, the broken record is the finding.

**Done for a walkthrough** is the same as for an audit: the root of the walkthrough is solid when every journey and every surface has been judged by its stated method, and a journey thought of later is a new leaf that reopens it. What the chain cannot yet say is *who* judged — a builder's verify and a tester's look alike, and independence lives in the evidence prose. An actor on the event is the one mechanism this section leaves open, until the first runs show whether prose is enough.
