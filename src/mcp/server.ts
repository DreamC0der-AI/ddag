...
2. Decompose by PROPERTY, never by topic ("a revoked device cannot read the vault", not "authz"), down to the method that settles each leaf: the leaf's Verify: names the method (crash injection after every write; an interleaving test; a fuzz harness; a review of named functions) and the result that counts. Write the whole tree BEFORE reporting any finding — the pending leaves are the audit plan.
3. Work the frontier bottom-up. A property that holds: verify it valid, citing what was run and the files. One shown false: refute it, then issue_open the finding on that property — a finding always lands on the property it refutes, never on a group.
